  ...
  container.innerHTML = `
    <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 16px;">
      ...
      <div style="flex: 1; text-align: center;">
        ...
        <div style="font-size: 12px; color: #6B7280;">Toplam: ${DateUtils.formatMinutes(weekTotalMinutes)}</div>
      </div>
      <button class="btn btn-outline btn-sm" onclick="changeWeeklyPlanWeek(1)">
    ...
    <div class="card">
      ...
      <form id="weeklyPlanForm" onsubmit="saveWeeklyPlan(event)">
        <div class="form-group">
          <label class="form-label">Öğrenci</label>
          <select class="form-select" id="planStudent" required onchange="loadStudentPlan()">
              ...
              <tr style="${day.isToday ? 'background: rgba(59, 130, 246, 0.05);' : ''}">
                ...
                <td>
                  <span class="badge ${day.totalMinutes > 0 ? 'badge-success' : 'badge-warning'}">
                    ${DateUtils.formatMinutes(day.totalMinutes)}
                  </span>
                </td>
                <td style="font-size: 12px; color: #6B7280;">${day.logCount} kayıt</td>
              </tr>
            `).join('')}
            <tr style="font-weight: 600; background: #F9FAFB;">
              <td>Toplam</td>
              <td><span class="badge badge-primary">${DateUtils.formatMinutes(weekTotalMinutes)}</span></td>
              <td style="font-size: 12px;">${dayTotals.reduce((sum, d) => sum + d.logCount, 0)} kayıt</td>
            </tr>
          </tbody>
        </table>
      </div>
...
async function addPlanTopic(date, topicId = '', minutes = 30) {
  ...
  div.innerHTML = `
    <select class="form-select" style="flex: 1; padding: 8px; font-size: 13px;" data-date="${date}" onchange="updatePlanDayTotal('${date}')">
    ...
    <input type="number" class="form-input" style="width: 70px; padding: 8px; font-size: 13px;" min="5" max="480" value="${minutes}" data-date="${date}" onchange="updatePlanDayTotal('${date}')">
    <span style="font-size: 12px; color: #6B7280;">dk</span>
    <button type="button" class="btn btn-outline btn-sm" style="padding: 4px; color: var(--danger);" onclick="this.parentElement.remove(); updatePlanDayTotal('${date}');">
...
function updatePlanDayTotal(date) {
  ...
  document.getElementById(`planDayTotal_${date}`).textContent = DateUtils.formatMinutes(total);
}
